...
function __pb10k_prompt_user_info {
  ...
  if [[ $SSH_CLIENT ]]; then
    ...
    printf "${_omb_prompt_yellow}$user${_omb_prompt_green}@|${_omb_prompt_cyan}$host"
  fi
}

function __pb10k_prompt_dir {
  local color=$_omb_prompt_yellow
  local box=""
  local icon=" "
  printf "${_omb_prompt_yellow}$icon ${_omb_prompt_bold_red}\w"
}
